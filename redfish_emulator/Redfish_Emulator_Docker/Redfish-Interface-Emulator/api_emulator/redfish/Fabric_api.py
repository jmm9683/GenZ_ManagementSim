...
class FabricAPI(Resource):
    ...
    # HTTP GET
    def get(self, ident):
        logging.info('FabricAPI GET called')
        try:
    ...
    def put(self, ident):
        ...
        return 'PUT is not a supported command for FabricAPI', 405
    ...
    def patch(self, ident):
        logging.info('FabricAPI PATCH called')
    ...
    def delete(self, ident):
        logging.info('FabricAPI DELETE called')
        try:
            # Find the entry with the correct value for Id
            resp = 404
            if ident in members:
...
class FabricCollectionAPI(Resource):
    ...
    def __init__(self):
        logging.info('FabricCollectionAPI init called')
        self.rb = g.rest_base
        self.config = {
            '@odata.context': self.rb + '$metadata#FabricCollection.FabricCollection',
            '@odata.id': self.rb + 'Fabrics',
            '@odata.type': '#FabricCollection.1.0.0.FabricCollection',
    ...
    # HTTP GET
    def get(self):
        logging.info('FabricCollectionAPI GET called')
        try:
    ...
    # HTTP PUT
    def put(self):
        logging.info('FabricCollectionAPI PUT called')
        return 'PUT is not a supported command for FabricCollectionAPI', 405
    ...
    # HTTP PATCH
    def patch(self):
        logging.info('FabricCollectionAPI PATCH called')
        return 'PATCH is not a supported command for FabricCollectionAPI', 405

    # HTTP DELETE
    def delete(self):
        logging.info('FabricCollectionAPI DELETE called')
        return 'DELETE is not a supported command for FabricCollectionAPI', 405
...
class CreateFabric(Resource):
    ...
    def __init__(self, **kwargs):
        logging.info('CreateFabric init called')
    ...
    # Create instance
    def put(self, ident):
        logging.info('CreateFabric put called')
        try:
            global config
            global wildcards
            wildcards['id'] = ident
            ...
            traceback.print_exc()
            resp = INTERNAL_ERROR
        logging.info('CreateFabric init exit')
        return resp
